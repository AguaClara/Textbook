...
\item 1) )Set up the airflow control hardware (Figure \eqref{ZEqnNum586787}).
 ...
 \item 2) )Set the Process Controller to operator selected state.

 \item 3) )Set the state to ``empty accumulator.''

 \item 4) )If desired open the cap of the accumulator to rapidly release any stored air.

 \item 5) )Set the Process Controller to automatic mode.

 \item 6) )The process controller should quickly cycle through the calibration step and then begin attempting to control the air flow rate to the target value.
\end{enumerate}
